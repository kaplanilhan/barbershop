...
import { MapPin, Phone, Clock, MessageCircle, User, AtSign, FileText, Send, Calendar, AlertCircle, CheckCircle } from 'lucide-react'
          ...
          <div className="contact-form">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="absolute -left-[9999px]" aria-hidden="true">
                ...
                <input id="website" name="website" value={formState.data.website} onChange={handleInputChange} tabIndex={-1} autoComplete="off" />
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="form-group">
                  <label htmlFor="preferredDate" className="label">Wunschdatum (optional)</label>
                  <input id="preferredDate" type="date" name="preferredDate" min={new Date().toISOString().split('T')[0]} value={formState.data.preferredDate} onChange={handleInputChange} className={`input ${formState.errors.preferredDate ? 'border-red-500' : ''}`} disabled={formState.isLoading} />
                  {formState.errors.preferredDate && <p className="mt-1 text-sm text-red-500">{formState.errors.preferredDate}</p>}
                </div>
                <div className="form-group">
                  <label htmlFor="preferredTime" className="label">Wunschzeit (optional)</label>
                  <input id="preferredTime" type="time" name="preferredTime" value={formState.data.preferredTime} onChange={handleInputChange} className={`input ${formState.errors.preferredTime ? 'border-red-500' : ''}`} disabled={formState.isLoading} />
                  {formState.errors.preferredTime && <p className="mt-1 text-sm text-red-500">{formState.errors.preferredTime}</p>}
                </div>
              </div>
